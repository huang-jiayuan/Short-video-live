...
func (v *VideoUser) Find(mobile string) error {
	return global.DB.Where("mobile=?", mobile).Find(&v).Error
}

func (v *VideoUser) Create(in *__.LoginRequest) error {
	u := &VideoUser{Mobile: in.Mobile}
	return global.DB.Debug().Create(&u).Error
}

func (v *VideoUser) ValidatePhoneNumber(mobile string) bool {
...
func (v *VideoUser) GetUserById(id int64) error {
	return global.DB.Where("id=?", id).First(&v).Error
}

func (v *VideoUser) ListUser(id int64) (list []*VideoUser, err error) {
	err = global.DB.Debug().Where("id =?", id).Find(&list).Error
	if err != nil {
		return nil, err
	}
	return
