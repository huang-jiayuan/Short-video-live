...
func (w *VideoWorks) GetVideoWorksById(id int64) error {
	return global.DB.Debug().Where("user_id =?", id).Find(&w).Error
}

func (w *VideoWorks) GetListVideoWorksById(id int64) (list []*VideoWorks, err error) {
	err = global.DB.Debug().Where("user_id =?", id).Find(&list).Error
	if err != nil {
		return nil, err
	}
	return
